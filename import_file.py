...
def import_file(name):
    ...
    for obj in objects.keys():
        for i in range(len(objects[obj])):
            for m in range(3):
                ...
                objects[obj][i][m] = [0 for i in range(3)]
                for j in range(3):
                    objects[obj][i][m][j] = float(vertices[int(vertix_index) - 1][j])
            objects[obj][i].append([0 for i in range(3)])
            for k in range(3):
                objects[obj][i][3][k] = float(normal[int(normal_index)-1][k])
            objects[obj][i][3] /= np.linalg.norm(objects[obj][i][3])
    ...
    for i in range(len(faces)):
        ...
        for j in range(len(faces[0])-2):
            ...
            for k in range(3):
                scene[i][j][k] = float(vertices[int(vertix_index)-1][k])
        for k in range(3):
